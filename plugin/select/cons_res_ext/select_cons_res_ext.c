/*****************************************************************************\
 *  select_cons_res_ext.c - node selection plugin supporting consumable
 *  resources policies that calls an external allocator.
 *****************************************************************************\
 *  To get more information on how a consumable resource policy works, see 
 *  the cons_res plugin
 * 
 *****************************************************************************
 ...
 */
const char plugin_name[] = "External allocator with CR Node Selection plugin";
const char plugin_type[] = "select/cons_res_ext";
const uint32_t plugin_id      = 107;
const uint32_t plugin_version = 100;
const uint32_t pstate_version = 7;	/* version control on saved state */

uint16_t cr_type = CR_CORE; /* cr_type is overwritten in init() */
...
static void _create_part_data(void)
{
	...
	if (!num_parts)
		return;
	info("cons_res_ext: preparing for %d partitions", num_parts);
			       ...
			       struct job_record *job_ptr)
{
	...
	debug3("cons_res_ext: build_row_bitmaps reshuffling %u jobs", num_jobs);
	...
	if (j < num_jobs) {
		...
		debug3("cons_res_ext: build_row_bitmap: dangling job found");
...
static int _add_job_to_res(struct job_record *job_ptr, int action)
{
	...
	debug3("cons_res_ext: _add_job_to_res: job %u act %d ", job_ptr->job_id,
	       action);
	...
	for (i = 0, n = -1; i < select_node_cnt; i++) {
		...
		if (action != 2) {
			...
			if ((select_node_usage[i].alloc_memory >
			     select_node_record[i].real_memory)) {
				error("cons_res_ext: node %s memory is "
	...
	if (action != 1) {
		...
		if (!p_ptr) {
			error("cons_res_ext: could not find cr partition %s",
		...
		for (i = 0; i < p_ptr->num_rows; i++) {
			if (!_can_job_fit_in_row(job, &(p_ptr->row[i])))
				continue;
			debug3("cons_res_ext: adding job %u to part %s row %u",
		...
		if (i >= p_ptr->num_rows) {
			...
			error("cons_res_ext: ERROR: job overflow: "
		       ...
		       struct job_record *to_job_ptr)
{
	...
	if (from_job_ptr->job_id == to_job_ptr->job_id) {
		error("select/cons_res_ext: attempt to merge job %u with self",
		      from_job_ptr->job_id);
		return SLURM_ERROR;
	}
	...
	if ((from_job_resrcs_ptr == NULL) ||
		...
		error("select/cons_res_ext: job %u lacks a job_resources struct",
		      from_job_ptr->job_id);
		return SLURM_ERROR;
	}
			    ...
			    struct job_record *job_ptr, int action)
{
	...
	debug3("cons_res_ext: _rm_job_from_res: job %u action %d", job_ptr->job_id,
	       action);
	if (select_debug_flags & DEBUG_FLAG_CPU_BIND)
		_dump_job_res(job);
	...
	if (action != 1) {
		...
		if (!job_ptr->part_ptr) {
			error("cons_res_ext: removed job %u does not have a "
			      "partition assigned",
			      job_ptr->job_id);
			return SLURM_ERROR;
		}
		...
		if (!p_ptr) {
			error("cons_res_ext: removed job %u could not find part %s",
		...
		for (i = 0; i < p_ptr->num_rows; i++) {
			...
			for (j = 0; j < p_ptr->row[i].num_jobs; j++) {
				if (p_ptr->row[i].job_list[j] != job)
					continue;
				debug3("cons_res_ext: removed job %u from "
				       "part %s row %u",
				       job_ptr->job_id,
		...
		if (n) {
			...
			for (i = 0, n = -1; i < select_node_cnt; i++) {
				...
				if (node_usage[i].node_state >=
					...
					error("cons_res_ext:_rm_job_from_res: "
				 ...
				 struct node_record *node_ptr)
{
	...
	if (!job || !job->core_bitmap) {
		error("select/cons_res_ext: job %u has no select data",
		      job_ptr->job_id);
		return SLURM_ERROR;
	}

	debug3("cons_res_ext: _rm_job_from_one_node: job %u node %s",
	       job_ptr->job_id, node_ptr->name);
	if (select_debug_flags & DEBUG_FLAG_CPU_BIND)
		_dump_job_res(job);
	...
	for (i = first_bit, n = -1; i <= last_bit; i++) {
		...
		if (node_usage[i].alloc_memory < job->memory_allocated[n]) {
			error("cons_res_ext: node %s memory is underallocated "
			      "(%u-%u) for job %u",
	...
	if (!job_ptr->part_ptr) {
		error("cons_res_ext: removed job %u does not have a partition "
	...
	for (i = 0; i < p_ptr->num_rows; i++) {
		...
		for (j = 0; j < p_ptr->row[i].num_jobs; j++) {
			if (p_ptr->row[i].job_list[j] != job)
				continue;
			debug3("cons_res_ext: found job %u in part %s row %u",
			       job_ptr->job_id, p_ptr->part_ptr->name, i);
	...
	if (n == 0) {
		error("cons_res_ext: could not find job %u in partition %s",
	...
	if (node_usage[node_inx].node_state >= job->node_req) {
		...
		error("cons_res_ext:_rm_job_from_one_node: node_state miscount");
	...
	if (rc != SLURM_SUCCESS) {
		...
		while ((tmp_job_ptr = list_next(job_iterator))) {
			...
			if (ovrlap == 0)	/* job has no usable nodes */
				continue;	/* skip it */
			debug2("cons_res_ext: _will_run_test, job %u: overlap=%d",
...
extern int select_p_node_init(struct node_record *node_ptr, int node_cnt)
{
	...
	info("cons_res_ext: select_p_node_init");
			     ...
			     bitstr_t *exc_core_bitmap)
{
	...
	if (select_debug_flags & DEBUG_FLAG_CPU_BIND) {
		info("cons_res_ext: select_p_job_test: job %u node_req %u mode %d",
		     job_ptr->job_id, job_node_req, mode);
		info("cons_res_ext: select_p_job_test: min_n %u max_n %u req_n %u "
		     "avail_n %u",
...
extern int select_p_reconfigure(void)
{
	...
	info("cons_res_ext: select_p_reconfigure");
	select_debug_flags = slurm_get_debug_flags();
